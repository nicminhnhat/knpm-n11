...
app.post("/api/uploads", authenticateToken, upload.single("file"), (req, res) => {
  if (!req.file) return fail(res, 400, "Vui lòng chọn hình ảnh cần tải lên.");
  const baseUrl = `${req.protocol}://${req.get("host")}`;
  return ok(res, { message: "Tải hình ảnh thành công.", url: `${baseUrl}/uploads/${req.file.filename}` }, 201);
});
